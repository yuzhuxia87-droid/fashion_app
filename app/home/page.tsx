...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        try {
          weatherData = await getWeatherByLocation(35.689, 139.692); // Tokyo
        ...
        console.log('[HOME] Fetching recommendations...');
        const response = await fetch(`/api/recommendations?${params}`);

        if (!response.ok) {
          console.error('[HOME] Recommendations fetch failed:', response.status);
          throw new Error('Failed to fetch recommendations');
        }

        const data = await response.json();
        console.log('[HOME] Recommendations fetched:', data.recommendations?.length || 0);

        const recommendations: OutfitWithStats[] = data.recommendations || [];
        setRecommendedOutfit(recommendations[0] || null);
        setAlternativeOutfits(recommendations.slice(1, 4));
      } catch (error) {
        console.error('[HOME] Initial data fetch error:', error);
  ...
  const handleDecide = async () => {
    ...
    try {
      ...
      console.error('[HOME] Record wear error:', error);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 pb-24 md:pb-8 space-y-6 md:space-y-8">
        ...
        {recommendedOutfit ? (
          <div className="space-y-4 md:space-y-5">
            ...
            {/* Buttons floating on gradient background */}
            <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
              <Button variant="outline" className="flex-1" onClick={handleDecide}>
            ...
            <CardContent className="p-5 md:p-6 pt-0">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-5">
                {alternativeOutfits.map((outfit) => (
                  <div
                    key={outfit.id}
                    className="relative aspect-[3/4] overflow-hidden rounded-2xl bg-gray-100 cursor-pointer group"
